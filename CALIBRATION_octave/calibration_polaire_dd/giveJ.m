function J = giveJ(gains, sky, na, n_dir, n_pola)
    J = zeros(n_pola*na*(na-1), n_pola*na*n_dir); %initialize an empty matrix at the right size
    ...
    for i = 1:na                               
      for j = 1:(na-1)                      %for each baselines
        ...
        if (n_pola ~=1)                     % if we do not have a scalar polarisation
          for dir = 1:n_dir
            tmp = compute_tmp(gains, sky, q, dir, n_dir);   %compute one block component
            for l=1:n_pola                      %for each polarisation      
              ...
              J((i-1)*(na-1)*n_pola + (j-1)*n_pola+index_r + 1,n_dir*(i-1)*4+4*(dir-1) +index_c+1) = insert;
              J((i-1)*(na-1)*n_pola + (j-1)*n_pola +index_r + 2,n_dir*(i-1)*4+4*(dir-1)+index_c+2) = insert;
            end;
          end
